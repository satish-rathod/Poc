import AgoraManagerAudioVoice from "./agora_manager_audio_voice_effects.js";
...
window.onload = async () => {
  ...
  const agoraManager = await AgoraManagerAudioVoice(handleVSDKEvents, product);
  ...
  async function handleFiles() {
    ...
    audioFileTrack = await agoraManager.createAudioFileTrack(this.files[0]);
  }
  ...
  document.getElementById("audioMixing").onclick = async function () {
    ...
    if (isAudioMixing == false) {
      // Start audio buffer
      agoraManager.startProcessAudioBuffer(audioFileTrack, channelParameters);
      // Change the button text.
      document.getElementById("audioMixing").innerHTML = "Stop audio mixing";
      // Update the audio mixing state.
      isAudioMixing = true;
    } else {
      agoraManager.stopProcessAudioBuffer(audioFileTrack, channelParameters);
