...
import AgoraManagerCustomAudioVideo from "./agora_manager_custom_media.js";
...
window.onload = async () => {
  ...
  const agoraManager = await AgoraManagerCustomAudioVideo(
  ...
  document.getElementById("custom-audio-video").onclick = async function () {
    await agoraManager.startCustomVideoAndAudio(channelParameters);
  };
  if(product == "live")
  {
    createToggleSwitch();
